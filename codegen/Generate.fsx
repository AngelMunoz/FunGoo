...
// Preconditions:
//   - Goo is built (Goo/Goo/bin/Debug/net10.0/Goo.dll exists).
//   - goo-widgets is built (goo-widgets/src/Goo.Widgets/bin/Debug/net10.0/Goo.Widgets.dll exists).
...
// Goo.Widgets, built from the goo-widgets submodule. LoadFrom keeps the
// generator decoupled from any concrete widget type name.
let widgetsAsm =
  Assembly.LoadFrom(
    Path.Combine(
...
let widgetTypes =
  widgetsAsm.GetExportedTypes()
  |> Array.filter(fun t -> not(isNull(box t)) && t.IsValueType && not t.IsEnum)
...
/// Public instance fields typed Nullable<T>, grouped by widget struct.
/// Plain fields need no generated members: F# constructor property assignment
/// (`ActionButton(Label = "Reset")`) covers them, including delegates.
let widgetNullableGroups =
  widgetTypes
  |> Array.choose(fun t ->
    let fields =
      t.GetFields(instanceFlags)
      |> Array.filter(fun f ->
        not f.IsInitOnly
        && not f.IsLiteral
        && f.FieldType.IsGenericType
        && f.FieldType.GetGenericTypeDefinition() =
          typeof<Nullable<_>>.GetGenericTypeDefinition())
      |> Array.sortBy(fun f -> f.Name)

    if fields.Length = 0 then None else Some(t, fields))
  |> Array.sortBy(fun (t, _) -> t.FullName)
...
// Lowercase-initial member names would collide with the language when the
// field name lowercases to an F# keyword (e.g. Open -> open). Escape those.
let fsharpKeywords =
  set [
    "abstract"
    "and"
...
let widgetMemberName(f: FieldInfo) =
  let name = camelCase f.Name

  if Set.contains name fsharpKeywords then
    $"``%s{name}``"
  else
    name
...
let headerFrom source moduleName (nowarnCodes: string list) =
  let nowarnLines =
    nowarnCodes
  ...
  $"// GENERATED — DO NOT EDIT\n// %s{moduleName} generated by codegen/Generate.fsx from the %s{source} public surface.\n%s{nowarnBlock}"

let header moduleName (nowarnCodes: string list) =
  headerFrom "Goo" moduleName nowarnCodes

let widgetsDir =
  Path.Combine(__SOURCE_DIRECTORY__, "..", "FunGoo", "Widgets")
  |> Path.GetFullPath

let widgetsProjectDir =
  Path.Combine(__SOURCE_DIRECTORY__, "..", "FunGoo.Widgets") |> Path.GetFullPath

let writeFileTo dir fileName (content: string) =
  File.WriteAllText(Path.Combine(dir, fileName), content)
  printfn $"wrote %s{fileName} (%d{content.Length} chars)"

let writeFile fileName (content: string) =
  writeFileTo widgetsDir fileName content
...
let renderFsiFileFrom
  ...
  let content = headerFrom source moduleName [] + code
  ...
  let diagnostics =
    ...
    with :? Fantomas.Core.ParseException as e ->
      File.WriteAllText(Path.Combine(dir, $"{moduleName}.invalid.fsi"), content)
  ...
  File.WriteAllText(Path.Combine(dir, $"{moduleName}.g.fsi"), content)
...
let renderFsiFile
  ...
  (valueSlots: WidgetBuilder<SyntaxOak.ValNode> seq)
  =
  renderFsiFileFrom "Goo" dir moduleName types valueSlots
...
// ---------------------------------------------------------------------------
// FunGoo.Widgets — optional-value setters for nullable widget fields
// ---------------------------------------------------------------------------

let widgetNullableMember(t: Type, f: FieldInfo) =
  let elementType = f.FieldType.GetGenericArguments() |> Array.head
  ...
  Member(
    ...
    [
      ParenPat(
        TuplePat [
          ParameterPat("this", LongIdent(t.FullName))

          (ParameterPat("?value", LongIdent(fsType elementType)))
          |> _.attributes([ Attribute("Struct") ])
        ]
      )
    ],
    CompExprBodyExpr [
      OtherExpr("let mutable copy = this")
      OtherExpr(SetExpr($"copy.{f.Name}", "ValueOption.toNullable value"))
      OtherExpr("copy")
    ],
    t.FullName
  )
...
let widgetNullableFsiType(t: Type, fields: FieldInfo[]) =
  TypeDefn($"{t.Name}Extensions") {
    for f in fields do
      fsiMemberSlot
        (widgetMemberName f)
        [
          SignatureParameter("this", LongIdent(t.FullName))

          (SignatureParameter(
            "?value",
            fsiType(f.FieldType.GetGenericArguments() |> Array.head)
          ))
...
printfn
  "scans: %d children types, %d setter types (%d setters), %d try methods, %d implicits, %d widget nullable groups (%d members)"
  childrenTypes.Length
  setterGroups.Length
  (setterGroups |> Array.sumBy(fun (_, p) -> p.Length))
  tryMethods.Length
  implicitOperators.Length
  widgetNullableGroups.Length
  (widgetNullableGroups |> Array.sumBy(fun (_, f) -> f.Length))
...
Directory.CreateDirectory(widgetsProjectDir) |> ignore

let widgetSettersCode =
  Oak() {
    Namespace("FunGoo") {
      Module("Widgets") {
        Open("System")
        ...
        for (t, fields) in widgetNullableGroups do
          extensionClass $"{t.Name}Extensions" fields (fun f ->
            widgetNullableMember(t, f))
      }
    }
  }
  |> Gen.mkOak
...
writeFileTo
  widgetsProjectDir
  "Widgets.g.fs"
  (headerFrom "Goo.Widgets" "Widgets" [] + widgetSettersCode)

renderFsiFileFrom
  "Goo.Widgets"
  widgetsProjectDir
  "Widgets"
  [ for group in widgetNullableGroups -> widgetNullableFsiType group ]
  Seq.empty
